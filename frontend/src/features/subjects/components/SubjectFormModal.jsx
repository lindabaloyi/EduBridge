...
export default function SubjectFormModal({ subject, submitting, submitError, onSubmit, onClose }) {
  ...
  function handleSubmit(e) {
    ...
    onSubmit({ name: form.name.trim(), code: form.code.trim().toUpperCase() });
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <label className="block">
            <span className="block text-xs uppercase tracking-wide mb-1" style={{ color: "#6D6858", fontFamily: mono }}>
              Name <span style={{ color: rust }}>*</span>
            </span>
            <input value={form.name} onChange={set("name")} placeholder="Mathematics" style={inputStyle(errors.name)} />
            {errors.name && <span className="block text-xs mt-1" style={{ color: rust }}>{errors.name}</span>}
          </label>

          <label className="block">
            <span className="block text-xs uppercase tracking-wide mb-1" style={{ color: "#6D6858", fontFamily: mono }}>
              Code <span style={{ color: rust }}>*</span>
            </span>
            <input value={form.code} onChange={set("code")} placeholder="MATH" style={inputStyle(errors.code)} />
            {errors.code && <span className="block text-xs mt-1" style={{ color: rust }}>{errors.code}</span>}
          </label>
